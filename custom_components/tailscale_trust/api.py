...
def parse_device(
    ...
    advertised_routes = raw.get("advertisedRoutes")
    enabled_routes = raw.get("enabledRoutes")
    return TailscaleDevice(
        ...
        advertised_routes=_string_tuple(advertised_routes),
        enabled_routes=_string_tuple(enabled_routes),
        routes_available=isinstance(advertised_routes, list)
        and isinstance(enabled_routes, list),
    )
...
class TailscaleTrustClient:
        ...
        async with response:
            if response.status == 429:
                raise TailscaleTrustRateLimitError(
                    self._rate_limit_delay(self._retry_after_header(response))
                )
    ...
    def _rate_limit_delay(self, value: str | None) -> float:
        ...
        base = min(
            RATE_LIMIT_MAX_RETRY,
            RATE_LIMIT_DEFAULT_RETRY * (2 ** min(self._rate_limit_failures - 1, 4)),
        )
    ...
    async def async_list_devices(self) -> dict[str, TailscaleDevice]:
        """List all device fields, retrying once after an authentication failure."""
        for attempt in range(2):
            ...
            if status == 429:
                raise TailscaleTrustRateLimitError(self._rate_limit_delay(retry_after))
            if status >= 400:
                raise TailscaleTrustConnectionError(
